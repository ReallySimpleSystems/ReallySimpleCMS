...
class Login {
	...
	private function sessionExists(string $session): bool {
		...
		return $rs_query->selectRow('users', 'COUNT(session)', array(
			'session' => $session
		)) > 0;
	}
	...
	private function isValidPassword(string $login, string $password): bool {
		...
		if(str_contains($login, '@')) {
			$db_password = $rs_query->selectField('users', 'password', array(
				'email' => $login
			));
		} else {
			$db_password = $rs_query->selectField('users', 'password', array(
	...
	private function isBlacklisted(string $login): bool {
		...
		return $rs_query->select($this->tables[1], 'COUNT(name)', array(
			'name' => $login
		)) > 0;
	}
	...
	private function shouldBlacklist(string $ip_address, string $login): void {
		...
		$last_blacklisted_ip = $rs_query->selectField($this->tables[0], 'last_blacklisted_ip', array(
		...
		foreach($login_rules as $login_rule) {
			...
			if($failed_logins >= $login_rule['attempts']) {
				if(!$this->isBlacklisted($ip_address)) {
					$failed_attempts = $rs_query->select($this->tables[0], 'COUNT(*)', array(
						'ip_address' => $ip_address,
						'status' => 'failure'
					));
					...
					$logins = $rs_query->select($this->tables[0], array('DISTINCT', 'login'), array(
						'ip_address' => $ip_address
					));
		...
		$last_blacklisted_login = $rs_query->selectField($this->tables[0], 'last_blacklisted_login', array(
			'login' => $login
		), array(
			'order_by' => 'id',
			'order' => 'ASC',
	...
	private function getBlacklistDuration(string $name): int {
		...
		$blacklist = $rs_query->selectRow($this->tables[1], array('blacklisted', 'duration'), array(
			'name' => $name
		));
