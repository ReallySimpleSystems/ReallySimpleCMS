...
class Login {
	...
	public function __construct(int $id, string $action, string $page) {
		...
		if(getSetting('delete_old_login_attempts')) {
			$login_attempts = $rs_query->select($this->tables[0], array('id', 'date'));
		...
		<table class="data-table">
			...
			<tbody>
				...
				foreach($login_attempts as $login_attempt) {
					...
					$blacklisted = $rs_query->select($this->tables[1], 'COUNT(name)', array(
	...
	private function blacklistExists(string $name): bool {
		...
		return $rs_query->selectRow($this->tables[1], 'COUNT(name)', array(
			'name' => $name
		)) > 0;
	}
