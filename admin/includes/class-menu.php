 ...
 * - private getMenuCount(string $search): int
 */
class Menu extends Term implements AdminInterface {
	...
	private function createMenuItem(string $type, int $id, int $index): array {
		...
		if($type === 'post') {
			$post = $rs_query->selectRow('posts', array('id', 'title'), array(
				'id' => $id
			));
			...
			$term = $rs_query->selectRow($this->table, array('id', 'name'), array(
				'id' => $id
			));
	...
	public function deleteMenuItem(int $id): void {
		...
		$parent = (int)$rs_query->selectField('posts', 'parent', array(
			'id' => $id
		));
	...
	private function validateMenuItemSubmission(array $data, int $id): string {
		...
		$menu = $rs_query->selectField('term_relationships', 'term', array(
			'post' => $id
		));
	...
	public function pageHeading(): void {
		...
		<div class="heading-wrap">
			...
			if(!empty($this->action)) {
				...
				// Record count
				if(!is_null($search))
					$count = $this->getMenuCount($search);
				else
	...
	private function isDescendant(int $id, int $ancestor): bool {
		...
		do {
			$parent = $rs_query->selectField('posts', 'parent', array(
				'id' => $id
			));
	...
	private function getMenuItems(): void {
		...
		<ul class="item-list">
			...
			foreach($itemmeta as $meta) {
				$menu_item = $rs_query->selectRow('posts', array('id', 'title', 'status', 'parent'), array(
					'id' => $meta['post']
				));
					...
					if(isset($_GET['item_id']) && (int)$_GET['item_id'] === $menu_item['id']) {
						if(isset($_GET['item_action'])) {
							switch($_GET['item_action']) {
								case 'move_up':
									$this->moveUpMenuItem($menu_item['id']);
									break;
								case 'move_down':
									$this->moveDownMenuItem($menu_item['id']);
									break;
								case 'edit':
									// Cancel button
									echo domTag('div', array(
	...
	private function getMenuItemsList(int $id, string $type): string {
		...
		if($type === 'post') {
			$post_type = $rs_query->selectField('posts', 'type', array(
				'id' => $id
			));
	...
	private function getMenuItemMeta(int $id): array {
		...
		$itemmeta = $rs_query->select('postmeta', array('datakey', 'value'), array(
			'post' => $id
		));
	...
	private function getParent(int $id): int {
		...
		return (int)$rs_query->selectField('posts', 'parent', array(
			'id' => $id
		));
	}
	...
	private function getFamilyTree(int $id): int {
		...
		$menu_item_id = $rs_query->selectField('posts', 'id', array(
			'id' => $id
		));
	...
	private function getDescendants(int $id): void {
		...
		$children = $rs_query->select('posts', 'id', array(
			'parent' => $id
		));
	 ...
	 * @return int
	 */
	private function getMenuCount(string $search = ''): int {
		global $rs_query;
